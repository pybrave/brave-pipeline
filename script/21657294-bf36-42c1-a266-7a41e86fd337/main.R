...
get_sig_df <- function(control, treatment,control_name){
...
df1 <- get_sig_df(control,treatment,control_name)


control2 <- data$control2 |>
  mutate(select_group= data$groups_name$control2)
treatment2 <- data$treatment2 |>
  mutate(select_group= data$groups_name$treatment2)
# log("microbiome control: ",paste0(control$sample_name,collapse = ", "))
# log("microbiome treatment: ",paste0(treatment$sample_name,collapse = ", "))

control_name2 <- data$groups_name$control2

df2 <- get_sig_df(control2,treatment2,control_name2)



common_samples <- intersect(colnames(df1),colnames(df2))

msg <- paste0("df1 sample size: ",length(colnames(df1)), " df2 sample size: ",length(colnames(df2)))
msg
log(msg)
