...
df_res |> write_tsv(file = paste0(output_path,"/",str_replace_all(title," ","_"),".tsv"))
...
ggplot(all_results, aes(x= coef, 
  ...
  geom_text_repel(data=all_results %>% filter(direction!="NS") |> head(label_size),aes(label = Pathway_Name), 
                  colour = "black", size = 4)
...
log2(merged_df[head(feature_list, n=30),]+1) |>
  ...
  left_join(select(anno,term,Pathway_Name),by="term") |>
  select(-term) |>
...
p <- box_data |>
  inner_join(metadata,by="sample_name") |>
  ggplot( aes(x=Pathway_Name, y=abundance, fill=select_group)) +
...
if (data$boxplot_sig_label =="Maaslin2"){
  p=p+geom_text(
    data = sig_feature,
    aes(x = Pathway_Name, y = max(box_data$abundance), 
        label = p.signif),
    inherit.aes = FALSE
  ) 
}else{
...
p <- ggplot(bubbleDf, aes(x = coef, y = factor(Pathway_Name,levels = pathway_levels), color = -log10(sig_value))) +
  geom_point(aes(size = abs(scores))) +
  scale_color_gradient(low = "blue", high = "red") +
  labs(x = "Coefficient", y = "", color = "-log10(sig_value)", size = "Score") +
  theme_minimal()
...
# 
# microbiome <- t(merged_df) |> as.data.frame() |>
#   select(where(~ any(. != 0)))
# 
# pheno <- list_path[c("sample_name",data$phenotype)] |>
#   column_to_rownames("sample_name")
# 
# identical(rownames(pheno),rownames(microbiome))
# 
# 
# pheno <- pheno %>%
#   imap(~{
...
# # 主要自变量 X
# # X <- rnorm(n)
# X <- factor(list_path$group)
# 
# # 函数：计算混杂效应
# confound_effect <- function(Y, X, Z) {
#   # 单变量回归
...
#   model2 <- glm(Y ~ X + Z, data=df, family = binomial)
#   beta2 <- coef(model2)[2]
# 
#   # 系数变化百分比
#   delta <- (beta1 - beta2)/beta1
#   return(delta)
# }
# model1 <- glm(X ~ microbiome[["map01501"]], family = binomial)
# coef(model1)[2] |> class()
# confound_effect( X,microbiome[["map01501"]], pheno[["Age"]])
# 
# results <- matrix(NA, nrow=ncol(microbiome), ncol=ncol(pheno),
#                   dimnames = list(colnames(microbiome), colnames(pheno)))
# 
# for (otu in colnames(microbiome)) {
#   for (z in colnames(pheno)) {
#     results[otu, z] <- confound_effect( X,microbiome[[otu]], pheno[[z]])
#   }
# }
# pheatmap(results,show_rownames = F)
