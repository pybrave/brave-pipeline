...
# term_annotation <- data$gbm_annotation$path
# 
...
df_list <- apply(list_path,1, function(x){
  profile_path <- x[["pathabundance_rpkm"]]
...
# read_tsv("/ssd1/wy/workspace2/nextflow_workspace/289364b1-295c-4710-833e-d68ec7c8918e/e2e9e213-8fe9-4d21-a75f-42671c865807/111b9ba1-64ad-4e47-a717-1af5b6af0e2a/output/humann/ACC1/pathabundance/ACC1_pathabundance.rpkm.tsv")|>
#   `colnames<-`(c("term","abundance")) |>
#   filter(!grepl("\\|",term)) 
...
merged_df0 <- rownames_to_column(merged_df,"taxonomy")|>
  mutate(feature = make.names(rownames(merged_df)))


all_results <- fit_data$results |>
  inner_join(merged_df0,by="feature") |>
  select(-feature) |>
  dplyr::rename(feature=taxonomy)
...
df_res |>
  relocate(feature,.before ="metadata" )|>
  relocate(scores,.after = "feature")|>
...
p <- box_data |>
  inner_join(metadata,by="sample_name") |>
  ggplot( aes(x=term, y=abundance, fill=select_group)) +
...
term_levels <- unique(c(sig_feature$feature))
p <- ggplot(sig_feature, aes(x = coef, y = factor(feature,levels = term_levels), color = -log10(sig_value))) +
  geom_point(aes(size = abs(scores))) +
  scale_color_gradient(low = "blue", high = "red") +
  labs(x = "Coefficient", y = "", color = "-log10(sig_value)", size = "Score") +
  theme_minimal()
