...
if(!is.null(data$query_metabolome) && data$query_metabolome!=""){
  ...
  intersect_names <- intersect(metabolome_list,rownames(microbiome_df))
  microbiome_df_res <- microbiome_df[intersect_names,]
  message(nrow(microbiome_df_res),"-",length(metabolome_list))
}
...
# rownames(experiment_df) |>paste0(collapse = ",")

# experiment_fit_data = Maaslin2(input_data     = t(experiment_df) |> as.data.frame(), 
