...
normalize_species <- function(x) {
  allowed <- c("hsa", "mmu", "rno", "dre", "ath", "sce", "cel", "dme","ko")
...
input_text <- as.character(params$deg %||% "")
feature_list_input <- trimws(unlist(strsplit(input_text, ",")))
...
feature_list <- unique(as.character(feature_list_input))
...
if (length(feature_list) > 0) {
  enrich_run <- run_kegg_enrichment(feature_list, species, candidates = gene_id_type)
...
info_lines <- c(
  ...
  "",
  "## Params",
  sprintf("- deg: %s", paste0(feature_list,collapse = ", ")),
  sprintf("- species: %s", species),
  sprintf("- gene_id_type: %s", gene_id_type),
  # sprintf("- top_n: %d", top_n),
  "",
  "## Stats",
  sprintf("- input_row_count: %d", length(feature_list)),
