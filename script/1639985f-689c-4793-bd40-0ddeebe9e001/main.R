...

{
  ...
  cat("",file = paste0(output_path,"/run.info"),append = F)
  log <- function(...){
...

{
  ...
  sig_num = sum(all_results$direction!="NS")
  all_results |>
    filter(direction!="NS") |>
    pull(feature) |>
    paste0(collapse = ",") %>%
    paste0(str_glue("{data$rank} level {sig_num} significantly different microorganisms: \n"),.) |>
    write(str_glue("{output_path}/diff.{sig_num}.txt"))
  ...
  ann_colors <- list(
    group = setNames(
      ...
      c(control_name, treatment_name)  # 用变量的值作为名字
    )
  )
  # dev.off()
  ...
  log2(merged_df[head(feature_list, n=30),]+1) |>
    pheatmap(scale = "row", 
             ...
             color =  colorRampPalette(c("darkblue", "#FFFFFF","darkred"))(255),
...
group_colors <- setNames(
  c(data$colors$control, data$colors$treatment),
...
p <- box_data |>
  ...
  inner_join(rownames_to_column(metadata,"sample_name"),by="sample_name") |>
  mutate(taxonomy=str_replace(taxonomy,"s__","")) |>
  ...
  labs(title=paste0("Boxplot of ",title), 
       ...
       color="Group",
       x="", 
       y=str_glue("Relative abundance of {tolower(rank)} log10")) +
  ...
  theme(
    ...
    panel.grid.major.y = element_line(linetype = "dashed"),   # 去掉竖网格线
    panel.grid.minor = element_blank(),  
    panel.border = element_blank(),        # 去掉整个边框
    axis.line = element_line(color = "black"),  # 只保留坐标轴线（下和左）
    legend.position = "right"                # 图例放上方
  )+ coord_flip()
p

if (data$boxplot_sig_label =="Maaslin2"){
  p=p+geom_text(
    data = sig_feature |> mutate(feature=str_replace(feature,"s__","")),
...
ggsave(filename = paste0(output_path,"/",str_replace_all(title," ","_"),"multigroup.boxplot.pdf"),plot = p,width = 10,height = 10)
