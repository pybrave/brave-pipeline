...
diff_metabolite <- function(name,prefix,feature_list=NULL){
  ...
  metabolite1 <- metabolite1[, colSums(is.na(metabolite1)) == 0]
  
  metadata1 <- rbind(params[[name]]$control, params[[name]]$treatment) |>
    select(sample_name,group=selcted_group_name)
  
  metadata1 <- metadata1 |>
    filter(sample_name  %in% colnames(metabolite1) )
  
  metabolite1_exp  <- metabolite1 |>
    select(feature, metadata1$sample_name) |>
    na.omit() 
...
pheatmap(
  ...
  fontsize_number = 10,
  fontsize = 12,
  main = "Behavior - Gene Correlation",
